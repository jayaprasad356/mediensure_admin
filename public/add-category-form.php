...
if (isset($_POST['btnAdd'])) {
        ...
        if (!empty($name)) {
            $result = $fn->validate_image($_FILES["category_image"]);
                ...
                // upload new image
                $upload = move_uploaded_file($_FILES['category_image']['tmp_name'], 'upload/categories/' . $menu_image);
        
                // insert new data to menu table
                $upload_image = 'upload/categories/' . $menu_image;

            
           
            $sql_query = "INSERT INTO categories (name,image)VALUES('$name','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_category'] = " <span class='label label-danger'>Failed add slide</span>";
            }
            }
        }
?>
