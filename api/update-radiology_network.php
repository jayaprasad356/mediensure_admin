...
$sql = "SELECT * FROM radiology_networks WHERE user_id='$user_id'";
$db->sql($sql);
$res = $db->getResult();
$num = $db->numRows($res);
if ($num >= 1) {
    $sql = "SELECT * FROM radiology_networks WHERE id=" . $inventory_id;
    $db->sql($sql);
    $res = $db->getResult();
    if (isset($_FILES['image']) && $_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0) {
        ...
        $sql = "UPDATE radiology_networks SET `image`='" . $upload_image . "' WHERE `id`=" . $inventory_id;
        $db->sql($sql);
    }
    $sql = "UPDATE radiology_networks SET center_name='$center_name',mobile='$mobile',email='$email',manager_name='$manager_name',center_address='$center_address',operational_hours='$operational_hours',latitude='$latitude',longitude='$longitude' WHERE id = '$inventory_id'";
    $db->sql($sql);
    $sql = "SELECT * FROM radiology_networks WHERE id=" . $inventory_id;
    $db->sql($sql);
    $res = $db->getResult();
    $response['success'] = true;
    $response['message'] = "Radiology Network Invetory Updated Successfully";
    $response['data'] = $res;
    print_r(json_encode($response));
    return false;
}
