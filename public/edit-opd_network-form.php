...
if (isset($_POST['btnEdit'])) {
        ...
        if (!empty($remarks)) {
			...
			$sql_query = "UPDATE opd_networks SET remarks = '$remarks', status = '$status' WHERE id =" . $ID;
			 $db->sql($sql_query);
			...
			// check update result
			if ($update_result == 1) {
				$error['update_network'] = " <section class='content-header'><span class='label label-success'>OPD Network updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM opd_networks WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "opd_networks.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit OPD Network<small><a href='opd_networks.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to OPD Networks</a></small></h1>
