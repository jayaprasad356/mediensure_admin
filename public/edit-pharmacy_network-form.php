...
if (isset($_POST['btnEdit'])) {
        ...
        if (!empty($remarks)) {
			...
			$sql_query = "UPDATE pharmacy_networks SET remarks = '$remarks', status = '$status' WHERE id =" . $ID;
			 $db->sql($sql_query);
			...
			// check update result
			if ($update_result == 1) {
				$error['update_network'] = " <section class='content-header'><span class='label label-success'>Pharmacy Network updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM pharmacy_networks WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "pharmacy_networks.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Pharmacy Network<small><a href='pharmacy_networks.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Pharmacy Networks</a></small></h1>
