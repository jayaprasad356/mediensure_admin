...
if (isset($_POST['btnEdit'])) {
        ...
        if (!empty($remarks)) {
			...
			$sql_query = "UPDATE LAB_networks SET remarks = '$remarks', status = '$status' WHERE id =" . $ID;
			 $db->sql($sql_query);
			...
			// check update result
			if ($update_result == 1) {
				$error['update_network'] = " <section class='content-header'><span class='label label-success'>LAB Network updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM lab_networks WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "lab_networks.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit LAB Network<small><a href='lab_networks.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to LAB Networks</a></small></h1>
