...
// else{
    ...
    $sql = "INSERT INTO radiology_networks (`user_id`,`center_name`,`mobile`,`email`,`manager_name`,`center_address`,`operational_hours`,`latitude`,`longitude`,`image`,`datetime`) VALUES ('$user_id','$center_name','$mobile','$email','$manager_name','$center_address','$operational_hours','$latitude','$longitude','$filename','$datetime')";
    $db->sql($sql);
    $sql = "SELECT * FROM radiology_networks WHERE center_name = '$center_name' AND email='$email'";
    $db->sql($sql);
    $res = $db->getResult();
    $response['success'] = true;
    $response['message'] = "Radiology Invetory Added Successfully";
    $response['data'] = $res;
    print_r(json_encode($response));
